...
def test_aggregates_match_between_xlsx_and_parquet(report_dir: Path):
    ...
    # read_full 은 라벨 컬럼을 category 로 반환(메모리 절감)하므로 groupby 결과가
    # CategoricalIndex 가 된다. 집계값 일치를 검증하되 인덱스 dtype/순서 차이는 허용.
    ga = a.groupby("매체상세", observed=True)["(스마트스토어센터기준) 전체 매출"].sum()
    gb = b.groupby("매체상세", observed=True)["(스마트스토어센터기준) 전체 매출"].sum()
    ga.index = ga.index.astype(str)
    gb.index = gb.index.astype(str)
    pd.testing.assert_series_equal(
        ga.sort_index(), gb.sort_index(), check_names=False
    )
